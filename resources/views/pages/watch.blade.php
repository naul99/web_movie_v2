...
<div style="padding-left: 2%">
    ...
    @endforeach
    @else
    <div id="server-{{ $ser->id }}" class="server{{ $server_active == 'server-' . $ser->id ? ' server-active' : '' }}"
    ...
    @foreach ($episode_movie as $key => $ser_mov)
    @if ($ser_mov->server_id == $ser->id)
    <div id="episodeList{{ $ser->id }}" class="episode-list" style="padding-left: 2%;padding-bottom: 1%">
        @foreach ($episode_list as $key => $ep)
        @if ($ep->server_id == $ser->id)
        <button data-url="{{ url('api/watch/' . $movie->slug . '/tap-' . $ep->episode . '/server-' . $ep->server_id) }}"
...
<section class="movieinformation container">
    <div class="">
        ...
        <img id="wishlist_movieimage" class="img-mobile" src="  @php
...
$image = substr($movie->movie_image->image, $startPos + strlen('movies/')); @endphp
    ...
    <div class="castinformation">
        <p><span class="name">Imdb Rating: </span>
            <a href="http://imdb.com/title/{{ $movie->imdb }}" target="_blank" rel="noopener noreferrer">{{ $values
                }}</a>
        </p>
    ...
    <div class="actions d-flex flex-start flex-middle wishlist">
        ...
        <a href="javacript:void(0)" id="shareButton" class="copy-url link-item" data-url='{{ url(' movie/' . $movie->slug) }}'>
    ...
    <div class="mylist-container d-flex flex-start flex-middle flex-no-wrap owl-carousel">
        ...
        <div class="video">
            <a href="javascript:void(0)"
                ...
                <img class="mylist-img p-r-10 p-t-10 video-item thumbnail-respone" src="
                            @php
...
<input type="hidden" id="witshlist_moviethumbnail" value="@php
    ...
    var name = document.getElementById('witshlist_moviename').value;
        ...
        function add_recent() {
            ...
            if (matchess.length) {
                ...
                matchess[0].thumbnail = thumbnail;
                matchess[0].url = url;
            } else {

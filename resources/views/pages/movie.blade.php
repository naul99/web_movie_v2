...
@section('content')
    <div class="text-white">
        <div class="md:grid md:place-items-center"><img class="md:w-[74rem] md:rounded-b-md"
                src=" @php
$image_check = substr($movie_thumbnail->movie_image->image, 0, 5); @endphp
                                                                            @if ($image_check == 'https') {{ $movie_thumbnail->movie_image->image }}
                                                                            @else
...
$image_check = substr($movie->movie_image->image, 0, 5); @endphp
            ...
            alt="The Incident">
        <button href="#" onclick="location.href='{{ url('xem-phim/' . $movie->slug . '/tap-' . $episode_first->episode . '/server-' . $episode_first->server_id) }}'"
        ...
        <h1 class="mx-2 mt-1 text-sm md:ml-20 md:w-[50rem] md:text-lg">  @if (isset($movie->movie_tags))
            @php
                $tagss = [];
